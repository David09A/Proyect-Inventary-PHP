...
$resultado = $con->query($consulta);
        ...
        while($row = $resultado ->fetch_assoc()) {            
            $y = $this->GetY();
            $this->SetXY(10, $y);
            $this->MultiCell(25, 4, utf8_decode($row['nit']), 1, 'C');
            $this->SetXY(35, $y);
            $this->MultiCell(50, 4, utf8_decode($row['nom_empresa']), 1, 'C');
            $this->SetXY(85, $y);
            $this->MultiCell(50, 4, utf8_decode($row['telefono']), 1, 'C');
            $this->SetXY(135, $y);
            $this->MultiCell(35, 4, utf8_decode($row['correo']), 1, 'C');
            $this->SetXY(170, $y);
            $this->MultiCell(30, 4, utf8_decode($row['direccion']), 1, 'C');
        }
    }
...
$pdf->Output('Reporte de Proveedores SoftNova  '.date("d_m_Y_H_i_s"), 'I'); //El primer parámetro es para colocar el nombre del archivo al momento de ser descargado y el segundo parámetro es para abrir el archivo en el navegador con la opción para poder ser descargado
?>
