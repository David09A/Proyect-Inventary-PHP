...
class PDF extends FPDF{    
    ...
    function Body() {
        ...
        $consulta = "SELECT * FROM insumos_para_produccion";
        $resultado = $con->query($consulta);

        while($row = $resultado ->fetch_assoc()) {            
            $y = $this->GetY();
            $this->SetXY(10, $y);
            $this->MultiCell(80, 4, utf8_decode($row['nom_produccion']), 1, 'C');
            $this->SetXY(90, $y);
...
$pdf->Output('Reporte de Insumos P SoftNova  '.date("d_m_Y_H_i_s"), 'I'); //El primer parámetro es para colocar el nombre del archivo al momento de ser descargado y el segundo parámetro es para abrir el archivo en el navegador con la opción para poder ser descargado
?>
