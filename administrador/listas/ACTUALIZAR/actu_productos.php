...
<body>
	<div class="contain1">
		...
		error_reporting(E_ERROR | E_PARSE);
			...
			$actualizar = "UPDATE `productos` SET `nombre` = '$nombre', `presentacion` = '$presentacion', `descripcion` = '$descripcion', `ubicacion` = '$ubicacion', `precio_venta` = $precio_venta, `precio_produccion` = $precio_produccion, `cantidad` = $cantidad, `nom_empaque` = $nom_empaque, `nom_produccion` = $nom_produccion, `nom_materia_prima` = $nom_materia_prima WHERE `productos`.`id` = $ini";
			...
			else
			{
				...
				?>
				<a href="../list_prod_admin.php" style="color: #fff;"><h2>Volver</h2></a>
				<?php
			}
