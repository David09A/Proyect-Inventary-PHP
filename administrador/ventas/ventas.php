...
class PDF extends FPDF{    
    ...
    function Body() {
        ...
        $this->SetXY(75, $y); //El resultado 35 es la suma de la posición 10 y el tamaño del MultiCell de 25.
        $this->MultiCell(40, 4, utf8_decode("Cliente"), 1, 'C');
        $this->SetXY(115, $y); //El resultado 35 es la suma de la posición 10 y el tamaño del MultiCell de 25.
        ...
        while($row = $resultado ->fetch_assoc()) {            
            $y = $this->GetY();
            $this->SetXY(10, $y);
            $this->MultiCell(25, 4, utf8_decode($row['id_venta']), 1, 'C');
            $this->SetXY(35, $y);
            $this->MultiCell(40, 4, utf8_decode($row['usuario_venta']), 1, 'C');
            $this->SetXY(75, $y);
            $this->MultiCell(40, 4, utf8_decode($row['client_venta']), 1, 'C');
            $this->SetXY(115, $y);
            $this->MultiCell(40, 4, utf8_decode($row['valor_total']), 1, 'C');
            $this->SetXY(155, $y);
            $this->MultiCell(45, 4, utf8_decode($row['fecha_venta']), 1, 'C');
        }
    }
