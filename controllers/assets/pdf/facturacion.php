...
class PDF extends FPDF{    
    ...
    function Body() {
        ...
        $consulta="SELECT V.k_idsale, D.k_idsale, D.k_refprod, P.k_refprod, P.n_name_prod, P.v_valor_prod, D.v_cant_prod, D.v_total_prod_cant, V.v_sale_total
        FROM vt007d_sale AS D
        INNER JOIN vt006m_sale AS V ON D.k_idsale=V.k_idsale
        INNER JOIN pr005m_prod AS P ON D.k_refprod=P.k_refprod
        WHERE V.k_idsale=$idvt;";
        ...
        while($row = $resul ->fetch_assoc()) {       
            $y = $this->GetY();
            $this->SetXY(10, $y);
            $this->MultiCell(80, 4, utf8_decode($row['n_name_prod']), 1, 'C');
            $this->SetXY(90, $y);
            $this->MultiCell(22, 4, utf8_decode($row['v_cant_prod']), 1, 'C');
            $this->SetXY(112, $y);
            $this->MultiCell(40, 4, utf8_decode($row['v_valor_prod']), 1, 'C');
            $this->SetXY(152, $y);
            $this->MultiCell(48, 4, utf8_decode($row['v_total_prod_cant']), 1, 'C');         
        }

        $sentencia="SELECT k_idsale, v_sale_total FROM vt006m_sale WHERE k_idsale=$idvt";
        ...
        $this->Cell(48, 10, "Total ".$row2['v_sale_total'], 0, 1, 'C');
    }
...
$pdf->Output('FacturaDeVenta_#'.$idvt.'_GNR'.date("d_m_Y_H_i_s").'.pdf', 'I'); //El primer parámetro es para colocar el nombre del archivo al momento de ser descargado y el segundo parámetro es para abrir el archivo en el navegador con la opción para poder ser descargado
?>
