...
class PDF extends FPDF{    
    ...
    function Body() {
        ...
        $this->Cell(220, 10, "Gracias Por Su Compra", 0, 1, 'C');
        ...
        $this->SetXY(112, $y);
        $this->MultiCell(48, 4, utf8_decode("Precio Unitario"), 1, 'C');
       ...
       /* $consulta="SELECT O.id_venta, O.cod_producto, O.cant_prod, P.id, P.nombre, P.precio_venta, V.id_venta, V.valor_total
        FROM det_ventas O, productos P, ventas V
        WHERE o.id_venta = V.id_venta AND P.id = O.cod_prod ";
        //$consulta = "SELECT id_venta, cod_producto, cant_prod FROM det_ventas WHERE id_venta = $id";
        $resul = $con->query($consulta);*/
    
            $y = $this->GetY();
            $this->SetXY(10, $y);
            $this->MultiCell(80, 4, utf8_decode($producto->nombre), 1, 'C');
            $this->SetXY(90, $y);
            $this->MultiCell(22, 4, utf8_decode($producto->cantidad), 1, 'C');
            $this->SetXY(112, $y);
            $this->MultiCell(48, 4, utf8_decode($producto->precio_venta), 1, 'C');         
...
$pdf->Output('Reporte_GanJha'.date("d_m_Y_H_i_s"), 'I'); //El primer parámetro es para colocar el nombre del archivo al momento de ser descargado y el segundo parámetro es para abrir el archivo en el navegador con la opción para poder ser descargado
?>
